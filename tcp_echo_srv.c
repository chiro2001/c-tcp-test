...
int echo_rep(int sockfd) {
  ...
  do {
    ...
    do {
      ...
      // TODO
      // 此处计算read_amnt及len_to_read的值，注意考虑已读完和未读完两种情况，以及其它情况（此时直接用上面的代码操作，free(buf),并
      // return pin_h）
      already_read += res;
...
int main(int argc, char* argv[]) {
  ...
  while (!sig_to_exit) {
    ...
    // 以下代码紧跟accept()，用于判断accpet()是否因SIG_INT信号退出（本案例中只关心SIGINT）；也可以不做此判断，直接执行
    // connfd<0
    // 时continue，因为此时sig_to_exit已经指明需要退出accept()主循环，两种方式二选一即可。
    ...
    if (!fork()) {  // 子进程
      ...
      fp_res = fopen(fn_res, "wb");  // Write only， append at the tail. Open or
